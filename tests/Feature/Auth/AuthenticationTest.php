...
test('sellers users can authenticate using the login screen', function () {
    $user = User::factory()->state(['role_id' => RoleEnum::SELLER])->has(Seller::factory()->state(['company_id' => Company::first()->id]))->create();
...
test('clients users can authenticate using the login screen', function () {
    ...
    $user = User::factory()->state(['role_id' => RoleEnum::CLIENT])
...
test('clients users can not authenticate with invalid password', function () {
    ...
    $user = User::factory()->state(['role_id' => RoleEnum::CLIENT])
        ->has(\App\Models\Client::factory()
            ->state([
                'address_id' => \App\Models\Address::first()->id,
